...
P = np.linspace(0.1,10,10000,True) # period in days
ang_freq = (2*np.pi)/P
dP = 0.5*(P[1]-P[0])*u.day
pgram = lombscargle(time_jd, vr, ang_freq, normalize=True)
ft = 17.5
fig = plt.figure(figsize=(8,6))
plt.plot(ang_freq, pgram)
...
plt.savefig("periodogram_scipy.png",bbox_inches="tight")
plt.close()
freq = ang_freq[np.argmax(pgram)]
period = (2*np.pi)/freq
print("Possible Period of given time-series radial velocity data is : {} +/- {} Earth Days".format(np.round(period,4),np.round(dP,4)))
...
plt.xlabel("Time (in days)",fontsize=ft)
plt.ylabel("Radial Velocity (in m/s)",fontsize=ft)
plt.savefig("time_folded_rv_curve_scipy.png",bbox_inches="tight")
plt.close()
...
plt.yticks(fontsize=ft)
plt.grid()
plt.savefig("scipy_curve_fit_scatter.png",bbox_inches="tight")
plt.close()
...
plt.yticks(fontsize=ft)
plt.grid()
plt.savefig("scipy_curve_fit.png",bbox_inches="tight")
plt.close()
...
plt.ylabel("Radial Velocity (in m/s)",fontsize=ft)
plt.savefig("phase_folded_rv_curve_scipy.png",bbox_inches="tight")
plt.close()
